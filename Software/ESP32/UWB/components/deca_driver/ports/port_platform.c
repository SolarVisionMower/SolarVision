 ...
 * All rights reserved.
 *
 * @author DecaWave and Arnav Revankar
 */

// NOTE: Only read and write functions for SPI have been modified!
...
{ 
    ...
    uint32_t total_len = readlength + headerLength;
    ...
    memset(tx_buf + headerLength, 0, readlength);

    spi_transaction_t t;
    memset(&t, 0, sizeof(t));       // Zero out the struct
...
int writetospi(uint16_t headerLength, const uint8 *headerBuffer, uint32 bodylength, const uint8 *bodyBuffer)
{
    // uint8 * p1;
    // uint32 idatalength=0;
